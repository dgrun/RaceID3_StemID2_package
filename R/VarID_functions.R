...
#' @param bmethod Character string indicating the batch correction method. If "harmony", then batch correction is performed by the \pkg{harmony} package. Default is \code{NULL} and batch correction will be done by negative binomial regression. This option is currently disabled, since harmony is not available on CRAN.
...
pruneKnn <- function(expData,distM=NULL,large=TRUE,regNB=TRUE,bmethod=NULL,batch=NULL,regVar=NULL,offsetModel=TRUE,thetaML=FALSE,theta=10,ngenes=2000,span=.75,pcaComp=NULL,tol=1e-5,algorithm="kd_tree",metric="pearson",genes=NULL,knn=25,do.prune=TRUE,alpha=1,nb=3,no_cores=NULL,FSelect=FALSE,pca.scale=FALSE,ps=1,seed=12345,theta.harmony=NULL,...){
    ...
    if ( class(expData)[1] != "Seurat" ){
        ...
        if ( !is.null(batch) & !is.null(bmethod) ){
            if ( bmethod == "harmony" ){
                message("Harmony is currently not available on CRAN. Falling back to bmethod NULL.")
         ##       hflag  <- TRUE
         ##       hbatch <- batch
         ##       batch  <- NULL
            }
