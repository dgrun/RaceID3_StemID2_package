...
This package implements additional improvements in rare cell type detection, offers batch effect removal utilities, optional imputing of gene expression, and substantially decreases runtime as well as memory usage. We tested the method successfully on a dataset of ~50k cells. We now added VarID2 [@VarID2] to the package, replacing the previous method VarID [@VarID], which allows the quantification of local gene expression variability and the deconvolution into technical and biological noise components. The original functions of VarID are still available, but were excluded from the vignette. We recommend using VarID2 with additional functionalities, in particular, quantification of biological noise. VarID2 furthermore offers an alternative clustering approach to RaceID based on density clustering which has certain advantages and, in particular, permits handling much larger datasets, since there is no requirement of storing a distance matrix. This method can be fully integrated into a RaceID analysis (see section "VarID2").
...
- id: VarID2
  ...
  URL: 'https://doi.org/10.1101/2022.08.04.502776'
